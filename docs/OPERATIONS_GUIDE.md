...
- **יצירת סביבת דמו מלאה:** `python scripts/seed_demo.py` - ממלא את ה-DB בנתוני דמו, אנשי מקצוע ולידים (מצוין לבדיקת ה-Dashboard וה-Analytics).
- **ניקוי נתוני טסטים בלבד:** `python scripts/reset_test.py` (למשל `python scripts/reset_test.py 972501234567`) - מנקה מצבי טסט מ-Redis ולידים של לקוח ספציפי.
