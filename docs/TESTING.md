...
| `test_promote_workflow.py` | PRO-183 `promote-to-production.yml`: the `gate` step's shell is extracted verbatim from the parsed YAML and run under `bash` (empty/whitespace/short/valid/trimmed `skip_reason`, Hebrew character-not-byte counting under a forced `C` locale, delimiter-injection safety), plus structural pins (`skip_deploy_check` fully removed, `skip_reason` input shape, verification steps gate on `!= 'false'` not `== 'true'`, the `gate` step stays unconditional, `run-name` stays single-line and flags unverified runs, the no-baseline "healthy but unproven" reporting path) |
