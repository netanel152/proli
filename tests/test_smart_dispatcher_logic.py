...
def mock_workflow_dependencies():
    with patch(
        "app.services.workflow_service.ai", new_callable=MagicMock
    ) as mock_ai, patch(
        "app.services.workflow_service.users_collection"
    ) as mock_users, patch(
        "app.services.workflow_service.whatsapp"
    ) as mock_whatsapp, patch(
        "app.services.workflow_service.lead_manager"
    ) as mock_lm, patch(
        "app.services.workflow_service.leads_collection"
    ) as mock_leads, patch(
        "app.services.matching_service.users_collection", new=mock_users
    ), patch(
        "app.services.matching_service.leads_collection", new=mock_leads
    ), patch(
        "app.services.workflow_service.StateManager"
    ) as mock_state:
        ...
        mock_leads.find_one = AsyncMock(
            side_effect=lambda query, **kwargs: (
                mock_lead_data if "_id" in query else None
            )
        )
...
async def test_handover_to_pro_success(mock_workflow_dependencies):
    ...
    mock_whatsapp.send_message.assert_any_call(
        "user123", "Hello, I am Mario the Plumber."
    )
...
async def test_audio_transcription_flow(mock_workflow_dependencies):
    ...
    calls_to_pro_text = [
        ...
        if call[0][0] == pro_chat
    ]
    calls_to_pro_file = [
        call.kwargs.get("caption", "")
        for call in mock_whatsapp.send_file_by_url.call_args_list
        if call.args[0] == pro_chat
    ]
    ...
    transcription_msgs = [
        msg for msg in all_texts_to_pro if "Water is flowing everywhere" in msg
    ]
