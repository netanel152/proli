...
2. Check `.env` has all required vars: GEMINI_API_KEY, CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET. (PRO-86 removed `GREEN_API_INSTANCE_ID` / `GREEN_API_TOKEN` — `Settings` no longer declares them, and leftovers in an environment are ignored.)
...
4. Check `WHATSAPP_PROVIDER` (`dryrun` | `cloud`, default `dryrun`) against intent, and note whether `WHATSAPP_DRY_RUN=true` is overriding it — that combination means the deploy will transmit nothing. Until PRO-89 lands, `dryrun` is the only working value.
