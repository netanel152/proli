...
**Current status: 1359 passed, 58 skipped, 3 xfailed** (integration tests skipped when `MONGO_TEST_URI` is not set. The remaining 52 skips are cells of the PRO-83 state × input matrix — but not all of them are deliberate `N/A`: 15 (`defect-finish`, `defect-cancel`, `defect-price`) are dark because of the same tracked product defects the strict xfails document below, not by design. The xfails are three product defects the harness documents — see below).
