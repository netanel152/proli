...
- **Commands** (`.claude/commands/*.md`) — operator workflows invoked by name: `/take-issue`, `/triage`, `/test`, `/health`, `/logs`, `/simulate`, `/user-debug`, `/db-status`, `/deploy-check`, `/full-sync-docs`, `/cleanup-worktrees`, `/add-pro`, `/sla-test`, `/finops`.
- **Agents** (`.claude/agents/*.md`) — `code-reviewer`, `test-runner`, `test-writer`, `docs-syncer`, `flow-tracer`, `ux-reviewer`.
- **Hooks** (`.claude/settings.json` → `.claude/hooks/`, all through the cross-platform `run-hook.sh` launcher): `SessionStart` injects real branch/commit/dirty-tree state and, on Claude Code on the web only, installs the venv so tests and linters run; `PreToolUse(Bash)` blocks `rm -rf` on dangerous targets, redirects into `.env`, force-pushes to protected branches, commit/push while on `dev`/`production`, and mongo `dropDatabase`/`drop()`; `PreToolUse(Edit|Write)` blocks edits to `.env` and `.git/`; `PostToolUse(Edit|Write)` runs `black` and reports `flake8` on touched `.py` files; `Stop` refuses to end a turn that changed documented source (`app/`, `admin_panel/`, `scripts/`) without touching any `.md` — once, then it lets go — so the docs-syncer step below cannot be forgotten silently.
...
- After finishing a code-change task, delegate to the **docs-syncer** subagent (incremental mode) to update any `.md` files made stale by the diff — `CLAUDE.md`, `.claude/rules/`, `README.md` and `docs/`. The `Stop` hook refuses (once) to end a turn that changed `app/`, `admin_panel/` or `scripts/` without touching any `.md`; answer it by running the syncer or by stating why the docs are unaffected.
