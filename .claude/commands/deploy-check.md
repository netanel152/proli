...
4. Check ENVIRONMENT is one of `development` | `staging` | `production`. Accept `staging` and `production` for a deploy; warn only if it is `development` (or unset). Any other value is a hard fail — the app raises at startup.
