...
| `.claude/skills/<name>/SKILL.md` | a procedure Claude picks up by context (`description`, optional `paths`); supporting files beside it load on demand | `test_claude_config.py` (frontmatter, referenced paths exist) |
...
| `PostToolUse(Edit\|Write\|MultiEdit)` | `post-edit-format.py` | runs `black --quiet` on the touched `.py`, reports `flake8` findings (config in `.flake8`; CI enforces both) on stderr; silent no-op without the tools |
