...
| `test_check_pro_service_areas_workflow.py` | `check_pro_service_areas.yml`: the resolve step's shell is extracted verbatim from the parsed YAML and run under `bash` — `dev` resolves to staging, `production` to production, and a feature branch, an unset or empty `REF_NAME`, `" production "` and `production-hotfix` all refuse with no target written; the exit-code `case` block is sliced out and run standalone for codes 0/1/2 and an unmapped code; structural pins that the only input is the boolean `apply` (no free-text confirmation box can return), that the branch reaches the shell through `env:` rather than `${{ }}`, that the check step keys its token/environment on `steps.target.outputs.target`, that `--apply` is only ever reached through the `APPLY` gate, that the script's exit code is captured via `PIPESTATUS` rather than `tee`'s, and that `run-name` is a single line. The exit-code `case` block is sliced out and run standalone for 0/1/2/3 and an unmapped code |
