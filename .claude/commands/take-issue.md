...
**10. Commit & PR.** Stage the changes and write a commit message referencing $1 (e.g. `feat($1): <summary>`). **Then sync the base before opening the PR:** `git fetch origin dev` and merge `origin/dev` in, resolve anything that conflicts here where the tree and the suite are, and re-run the tests. Step 3 branched off the remote tip, but `dev` has moved since — a branch that has not seen it is where conflicts come from, discovered by whoever reviews the PR instead of by you. Merge, never rebase: a rebase invalidates every existing checkout of the branch. The `PreToolUse(Bash)` guard refuses the PR-open command from a branch that is behind, with that same sync command in its message. Then open the PR. **The `$1` in that title is a closing marker:** Linear links via branch name, PR title *or* PR body, and moves the issue to Done on merge. That is correct here, because this flow runs one issue to completion. If you ever finish only part of $1, keep the key out of all three and link the PR from a Linear comment instead — see the Linear ↔ GitHub section in `CLAUDE.md`. The PR body must include: a one-paragraph summary, the acceptance criteria checked off, the code-reviewer's verdict, and the test count as "<old from docs/TESTING.md> → <new>". If the change touched `app/core/constants.py` (new state, TTL, or lifecycle value) or the `workflow_service.py` dispatch order, no manual agent-file review is needed: the embedded agent-pack facts (UserStates, LeadStatus lifecycle, TTLs, and dispatch order) are guarded automatically by `tests/test_agent_pack_drift.py` — a failing run there points at the exact stale fact to fix.
...
**12. Record it in the living system-audit artifact — respecting the single-writer rule.** You cannot detect the other writers from here: a session in its own container shares the artifact but not the working tree, so `git worktree list` can read one while somebody else is mid-write (it misled twice on 2026-09-14, which is why it is no longer the check). **Never pass `force`** — the stale-refusal is what keeps two sessions from losing an entry, and hitting it costs only a re-read.

- **Publishing directly** — fine for a single row when nothing else is in flight. `action: "read"` the artifact **immediately before writing** and build your update from the version that comes back (never from a copy read earlier in the session), then republish it via the Artifact tool with `url: https://claude.ai/code/artifact/363c67d3-e33c-44f2-a8fd-afe6534711c7` (the `url` parameter is what updates it in place — omitting it forks a new artifact). Add a Fix-log row for $1 (date, one-line outcome, PR link) and, if $1 appears among the audit's "Filed in Linear" cards, mark that card done. Keep the title and 🩺 favicon unchanged.
- **Queueing it in the PR body** — the default whenever another session might be running (a parallel batch, more than one worktree, or simply not knowing). Put your Fix-log row in the PR body under a `## 🩺 Audit fix-log entry` heading and stop there — the session closing the batch drains every merged PR's section into one write. Say in your final status block that the row is queued, not published, so nobody assumes the dashboard is current.
