...
A CI step fails the build on any reference to the old vendor's domain (either spelling) in **code** — see the "Guard" step in `.github/workflows/tests.yml`, pinned by `tests/test_vendor_guard_workflow.py`. `*.md` is excluded, so prose may name the retired vendor plainly: a document is history, not an egress bypass. Code, scripts and fixtures still fail the build. A second "Guard — all outbound traffic through the provider facade" step fails the build on `httpx`/`requests` imports under `app/services/` (allowlisting `geocoding_service.py`) and on `CloudAPIProvider(`/`DryRunProvider(` construction anywhere in `app/` outside `app/providers/whatsapp/`.
...
- After completing any task/bug (PR opened or Linear issue moved), record it in the **living system-audit artifact** — but **exactly one session writes to it at a time.** It is a single shared document with no merge: a concurrent republish is refused as stale, and forcing past that refusal silently discards the other session's entry. **Never pass `force`** — that refusal is the only thing standing between two sessions and a lost entry, and it is safe to hit: you re-read and rebuild, nothing is damaged.
  
  **You cannot tell from here whether you are the only writer.** `git worktree list` used to be the check, and it is wrong: a session running in its own container shares the artifact but not the working tree, so it shows one worktree while somebody else is mid-write. That misread twice on 2026-09-14. Treat a second writer as always possible and pick by where your row is safest:
  - **Publishing directly** — fine for a one-off row. `action: "read"` the artifact **immediately** before writing and build the republish from the version that comes back (not from a copy read earlier in the session — that is how you arrive stale), then republish via the Artifact tool with `url: https://claude.ai/code/artifact/363c67d3-e33c-44f2-a8fd-afe6534711c7` (passing the URL is what updates in place; omitting it forks a new artifact). Add a Fix-log row (date, item, outcome + PR link) and update the affected item's status card. Keep the title ("Proli System Audit") and favicon (🩺) unchanged.
  - **Queueing it in the PR body** — the right default whenever another session might be running (a parallel batch, more than one worktree, or simply not knowing). Put your Fix-log row in the PR body, under a `## 🩺 Audit fix-log entry` heading, and stop there. Whoever closes the batch drains every merged PR's section into a single ordered write (`gh pr list --state merged --json number,body`), so the artifact takes one write instead of N racing ones — and no entry can be lost to a force. When in doubt, queue: a queued row costs one paste, a lost row is invisible.
