...
- **Chat id / free-text masking at the call site (PRO-191):** the sink-level filters above scrub the *rendered message* for phone- and street-shaped patterns; they don't know a raw `chat_id`, `full_address`, `customer_name`, `transcription` or similar field was ever there. A `chat_id` interpolated into a log line must go through `app/core/phone.py`'s `mask_chat_id`; the other fields shouldn't be logged at all (log presence/shape instead — a length, a boolean, a key list). `tests/test_log_pii.py` enforces this as an unconditional floor over `app/` and `admin_panel/`: zero violations anywhere, no per-file exemption (PRO-195 paid down the last 38 lines across 12 files and deleted the `KNOWN_VIOLATIONS` allowlist that used to carry them).
...
**Backfill check for pros approved before this existed.**

The supported path is the **`🗺️ Check pro service areas`** workflow (Actions tab → Run workflow). Like the stop workflow, the branch you run it from is the target — pick `production` to check production, `dev` to check staging, and any other branch refuses rather than touching a real database. It reads `MONGO_URI`/`REDIS_URL` straight from repo secrets rather than going through the Railway CLI (see "The credentials CI holds" in `docs/RAILWAY_SETUP.md` for why, and for the secrets it needs), fails rather than falling back to localhost when the database secret is missing, and prints the database name it reached so an empty result can be told apart from an empty *connection*. The one control is a checkbox: leave `apply` unticked for a read-only report, tick it to write `location` on pros that have none and record each pro's verdict. The report lands in the run's job summary, which is what you paste onto the issue — the script only proves the backfill once it has actually been run against production.

By hand, if you need it — run from the repo root with the target environment's `MONGO_URI`/`REDIS_URL` in your shell, and `GOOGLE_MAPS_API_KEY` set or every non-static name reports *unavailable* rather than *unresolved*. `railway run …` still works for **production**; the staging project token is currently refused by Railway (see `docs/RAILWAY_SETUP.md`), so export the values yourself for that one:
...
### Freshness watchdog (PRO-185)

The PRO-111 escalation above only fires when the job *runs*. A worker that is down at 02:00, a misfired cron tick, a stale scheduler lock, or the PRO-96 misconfiguration that never registers the job at all — none of those produce a failure, so none of them page.

- **`run_daily_backup` records success in two places.** On a zero exit it writes `backup:last_success` (unix timestamp, **no TTL**) in Redis and clears `backup:consecutive_failures` together with the watchdog's own bookkeeping keys, then — in its own fail-open block — upserts a durable mirror, `settings.backup_state` (`{"_id": "backup_state", "last_success": <aware UTC datetime>}`), in Mongo. Redis stays the primary read (cheap, replica-shared); the mirror exists because a no-TTL, rarely-read key is exactly what `allkeys-lru` eviction reaps first, and a Redis restart without persistence drops it the same way.
- **`run_backup_freshness_watchdog`** (job id `backup_freshness_watchdog`) runs hourly (`WorkerConstants.BACKUP_WATCHDOG_INTERVAL_MINUTES = 60`), plus once shortly after boot (PRO-176), **production only** — the same `settings.is_production` gate as the backup job itself. It pages `page_critical` when the last recorded success is older than `WorkerConstants.BACKUP_MAX_AGE_HOURS = 48`, deduped to once per `WorkerConstants.BACKUP_STALE_REALERT_HOURS = 24` (`backup:stale_alert`, SET NX EX). Redis is the common-path read; the mirror is consulted whenever Redis's `backup:last_success` is **absent or stale** — the two writes are independent fail-open blocks, so Redis can lag the mirror (a failed Redis write, a restore from an old snapshot), and a success recorded in either store counts. Absent-in-both starts the cold-start clock (`backup:watchdog_first_seen`), quiet until that seed is itself older than 48h — day one is silent, but a broken job behind a wiped Redis still pages eventually instead of never; stale-in-Redis-but-fresher-in-Mongo uses the mirror's freshness instead of paging. A corrupt `watchdog_first_seen` seed is reseeded (ERROR log) rather than left to silence the watchdog for good. An unparseable value, or one dated further in the future than `WorkerConstants.BACKUP_CLOCK_SKEW_TOLERANCE_SECONDS = 300` tolerates (worker-clock-writes/API-clock-reads skew), logs ERROR without paging; Redis unreachable logs WARNING and fails open, same posture as `_report_backup_failure`.
- **The `daily_backup` cron itself now has `coalesce=True` and `misfire_grace_time=SCHEDULER_LONG_JOB_MISFIRE_GRACE_SECONDS`** (600s), the same PRO-176 treatment as the other long-interval jobs.
- **The authenticated `/health` `checks` block carries a `backup` object** — `{"status": "fresh"|"stale"|"never"|"unknown", "last_success": ISO-8601 UTC or null, "age_hours": float or null, "max_age_hours": 48, "enabled": <settings.is_production>, "platform_environment": <RAILWAY_ENVIRONMENT_NAME or null>}`. `never` means neither Redis nor the Mongo mirror has a record; `unknown` means the store was unreachable, the value was unparseable, or `last_success` is out beyond the clock-skew tolerance. Same fallback as the watchdog: a stale or absent Redis value is checked against the mirror before settling on `stale`/`never`. The mirror lookup only runs for a caller `_detail_authorized` accepts — `detail_ok` is decided once at the top of the handler, so the Docker HEALTHCHECK and the PRO-155 staging poller, which never see `checks`, never pay the extra Mongo round-trip; an unauthenticated caller with no Redis key is simply treated as `never` internally. `platform_environment` is Railway's own, un-mistypable environment name — a synthetic monitor must not read `enabled: false` as "just staging" when the platform says `production`; that combination is the PRO-96 misconfiguration, and `"never"` is then the alarm. The top-level `status` is deliberately unaffected — freshness is an operator page, not a liveness signal, and the Docker HEALTHCHECK must not restart a healthy API over it.
- **What still can't be caught in-process:** a dead worker (it *is* the worker) and the PRO-96 case where the job is never registered. Both are visible only through `/health`'s `backup` block to an external caller — that's the load-bearing half of PRO-185.
