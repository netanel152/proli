...
| `test_check_pro_service_areas_workflow.py` | `check_pro_service_areas.yml`: the resolve step's shell is extracted verbatim from the parsed YAML and run under `bash` — `dev` resolves to staging, `production` to production, and a feature branch, an unset or empty `REF_NAME`, `" production "` and `production-hotfix` all refuse with no target written; the exit-code `case` block is sliced out and run standalone for codes 0/1/2 and an unmapped code; structural pins that the only input is the boolean `apply` (no free-text confirmation box can return), that the branch reaches the shell through `env:` rather than `${{ }}`, that the check step keys its token/environment on `steps.target.outputs.target`, that `--apply` is only ever reached through the `APPLY` gate, that the script's exit code is captured via `PIPESTATUS` rather than `tee`'s, and that `run-name` is a single line. The exit-code `case` block is sliced out and run standalone for 0/1/2/3 and an unmapped code. The credential step is covered the same way: an empty secret fails with an error naming the repository-secret vs. Environments-secret distinction, and a non-empty one reports a four-line fingerprint — real character count, the last four characters *of the trimmed value* (a trailing newline used to break the line mid-comparison, and a test pins that it does not), whether whitespace is present and how much, and whether the value is UUID-shaped as a Railway project token is. The disclosure is pinned to exactly four trailing characters: for a sentinel, the last 4 must appear and the last 5 and first 8 must not, so the bound cannot silently widen. One pin exists for a property rather than a behaviour: the step's shell must never invoke `railway` (word-boundary match, so the `RAILWAY_*` env names do not satisfy it). The probe that was there briefly used `railway whoami`, which a valid *project* token cannot pass — it failed the job before the real command ran. That test is what stops it coming back. One further pin: the `railway run` command must not carry an `--environment` flag — a project token is bound to one environment already, and passing the flag made the CLI attempt a project-level name lookup that an environment-scoped token is not permitted to do. The credential-name fallback is exercised against a stub `railway` on `PATH` for all four outcomes — the first name working with no retry, a real exit-3 finding *not* being mistaken for an auth failure, the retry under `RAILWAY_API_TOKEN` rescuing a refusal, and both names refused exiting before the exit-code interpretation so a credential failure is never reported as a finding about pros nobody looked at — plus that the retry carries the byte-identical token and that the command is built once into `CMD` so the two call sites cannot drift. Every extracted shell runs under `bash -e`, matching GitHub's `bash -e {0}`: without that the harness cannot see errexit aborting a step, which is how a credential retry, a job-summary write and an exit-code interpretation all sat in the workflow as dead code through a day of failing runs |
