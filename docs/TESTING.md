...
| `test_log_pii.py` | PRO-191 ratchet: an AST scan of every `logger`-bound f-string in `app/` and `admin_panel/` flags an unmasked `chat_id` or a structural reference to `full_address`/`street`/`street_number`/`customer_name`/`display_name`/`transcription`/`pro_phone`/`phone_number` (`city` is deliberately allowed); `KNOWN_VIOLATIONS` pins the remaining debt at exactly 38 across 12 files — zero in any unlisted file, none above its count, none below either — while `dispatch_guards.py`/`workflow_service.py` (fixed in this PR, standardized on `mask_chat_id`) must carry zero; the detector itself is pinned by unit tests on in-process source snippets |
