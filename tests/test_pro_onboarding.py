...
async def test_start_onboarding_new_user(onboard_mocks):
    ...
    mock_state.set_state.assert_called_with(
        "972501111111@c.us", UserStates.ONBOARDING_NAME
    )
    mock_wa.send_message.assert_called_once_with(
        "972501111111@c.us", Messages.Onboarding.WELCOME
    )
...
async def test_start_onboarding_already_registered(onboard_mocks):
    ...
    mock_wa.send_message.assert_called_once_with(
        "972501111111@c.us", Messages.Onboarding.ALREADY_REGISTERED
    )
...
async def test_start_onboarding_pending_approval(onboard_mocks):
    ...
    assert result is False
    mock_wa.send_message.assert_called_once_with(
        "972504444444@c.us", Messages.Onboarding.PENDING_ALREADY
    )
...
async def test_step_name_valid(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972501111111@c.us", "יוסי אינסטלציה", UserStates.ONBOARDING_NAME, mock_wa
    )
    ...
    mock_state.set_state.assert_called_with(
        "972501111111@c.us", UserStates.ONBOARDING_TYPE
    )
    mock_wa.send_message.assert_called_once_with(
        "972501111111@c.us", Messages.Onboarding.ASK_TYPE
    )
...
async def test_step_name_too_short(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972501111111@c.us", "א", UserStates.ONBOARDING_NAME, mock_wa
    )
...
async def test_step_type_valid_number(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972501111111@c.us", "1", UserStates.ONBOARDING_TYPE, mock_wa
    )

    saved_data = mock_state.set_metadata.call_args.args[1]
    assert saved_data["onboarding"]["type"] == "plumber"
    mock_state.set_state.assert_called_with(
        "972501111111@c.us", UserStates.ONBOARDING_AREAS
    )
...
async def test_step_type_valid_hebrew(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972501111111@c.us", "חשמלאי", UserStates.ONBOARDING_TYPE, mock_wa
    )
...
async def test_step_type_invalid(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972501111111@c.us", "99", UserStates.ONBOARDING_TYPE, mock_wa
    )

    mock_state.set_state.assert_not_called()
    mock_wa.send_message.assert_called_once_with(
        "972501111111@c.us", Messages.Onboarding.INVALID_TYPE
    )
...
async def test_step_areas(onboard_mocks):
    ...
    mock_state.get_metadata.return_value = {
        "onboarding": {"name": "Test", "type": "plumber"}
    }
    ...
    mock_state.set_state.assert_called_with(
        "972501111111@c.us", UserStates.ONBOARDING_PRICES
    )
...
async def test_step_areas_empty(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972501111111@c.us", "  ", UserStates.ONBOARDING_AREAS, mock_wa
    )
...
async def test_step_prices_with_text(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972501111111@c.us", "תיקון 250₪", UserStates.ONBOARDING_PRICES, mock_wa
    )

    saved_data = mock_state.set_metadata.call_args.args[1]
    assert saved_data["onboarding"]["prices"] == "תיקון 250₪"
    mock_state.set_state.assert_called_with(
        "972501111111@c.us", UserStates.ONBOARDING_CONFIRM
    )
...
async def test_step_prices_skip(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972501111111@c.us", "דלג", UserStates.ONBOARDING_PRICES, mock_wa
    )
...
async def test_step_confirm_yes(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972505555555@c.us", "אשר", UserStates.ONBOARDING_CONFIRM, mock_wa
    )

    mock_state.clear_state.assert_called_with("972505555555@c.us")
    mock_wa.send_message.assert_called_once_with(
        "972505555555@c.us", Messages.Onboarding.SUCCESS
    )

    # Verify pro created in DB
    pro = await db.users.find_one(
        {"phone_number": "972505555555", "role": "professional"}
    )
...
async def test_step_confirm_cancel(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972501111111@c.us", "ביטול", UserStates.ONBOARDING_CONFIRM, mock_wa
    )

    mock_state.clear_state.assert_called()
    mock_wa.send_message.assert_called_once_with(
        "972501111111@c.us", Messages.Onboarding.CANCELLED
    )
...
async def test_cancel_at_type_step(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972501111111@c.us", "ביטול", UserStates.ONBOARDING_TYPE, mock_wa
    )
...
async def test_cancel_at_areas_step(onboard_mocks):
    ...
    await handle_onboarding_step(
        "972501111111@c.us", "cancel", UserStates.ONBOARDING_AREAS, mock_wa
    )
