...
# PRO-182: no caller left *in this module* — the pipeline that matched a pro
# moved to conversation_pipeline.py, which reaches this as `wf.determine_best_pro`
# through a call-time import of this module (the A1 convention that keeps the
# suite's `monkeypatch.setattr(workflow_service, …)` points working). flake8
# cannot see that and reports it unused; deleting it on that advice breaks
# matching at runtime, not at import. Same for `detect_and_fetch_media` below.
from app.services.matching_service import determine_best_pro  # noqa: F401
...
from app.services.media_handler import detect_and_fetch_media  # noqa: F401
...
async def process_incoming_message(chat_id: str, user_text: str, media_url: str = None):
    ...
    if not acquired:
        logger.info(
            f"🔒 Chat lock held for {mask_chat_id(chat_id)} — another task is mid-flight; deferring"
        )
...
async def _accept_loyalty_offer(chat_id, past_pro_id, active_lead, meta) -> bool:
    ...
    if not past_pro:
        ...
        logger.info(
            f"Loyalty offer accepted by {mask_chat_id(chat_id)} but pro {past_pro_id} is "
    ...
    if not ok:
        ...
        logger.info(
            f"Loyalty accepted by {mask_chat_id(chat_id)} — pro {past_pro['_id']} saved, "
    ...
    if updated is None:
        logger.info(
            f"Loyalty dispatch for {mask_chat_id(chat_id)} lost the status race on lead "
    ...
    if not notified:
        ...
        logger.error(
            ...
            f"{mask_chat_id(chat_id)} — SLA monitor will recover the lead"
        )
    ...
    await lead_manager.log_message(chat_id, "model", ack)
    logger.info(
        f"Loyalty accepted by {mask_chat_id(chat_id)} — lead {active_lead['_id']} "
...
async def _execute_customer_cancel(booked_lead, chat_id: str) -> None:
    ...
    logger.info(
        f"Customer {mask_chat_id(chat_id)} cancelled BOOKED lead {booked_lead['_id']}"
    )
    if pro_id:
...
async def _escalate_emergency(chat_id: str, current_state, user_text: str = ""):
    ...
    if not active_lead:
        ...
        await StateManager.clear_state(chat_id)
        logger.info(
            f"🚑 Emergency released {_state_label(current_state)} for {mask_chat_id(chat_id)} "
            "with no "
    ...
    if not already_flagged:
        ...
        logger.warning(
            f"🚑 Emergency declared mid-conversation by {mask_chat_id(chat_id)} in state "
    ...
    if current_state == UserStates.AWAITING_ADDRESS:
        ...
        if known_city:
            logger.info(
                f"🚑 Emergency released the address gate for {mask_chat_id(chat_id)} "
...
async def _process_incoming_message_inner(
    ...
    # PRO-182 (PRO-139 slice B): everything the chain falls through to is a
    # straight line rather than a decision table — one message, top to bottom,
    # each step consuming what the one above it produced — so it lives in
    # `conversation_pipeline` as a function instead of as more guards.
    #
    # The post-chain values travel on `ctx`, not on this function's locals: the
    # guards mutate them on the way past (the rate limiter resolves `is_exempt`,
    # several guards refresh `current_state` before falling through, and the
    # emergency hoist sets `emergency_inbound_logged` so the pipeline's step 1
    # does not log the same turn twice), and the pipeline reads them there.
...
async def _finalize_deal(
    ...
    logger.info(
        f"🚧 Address gate check for {mask_chat_id(chat_id)}: city={ed.city!r}, "
        # Derived from the gate's own field set and using the gate's own
        # emptiness test (`is_address_complete` strips). A hardcoded tuple
        # would drift if a field were added, and without `.strip()` a
        # whitespace-only extraction reads as present and is then rejected —
        # exactly the case an operator is staring at when the gate "shouldn't"
        # have failed.
        f"present={[f for f in Messages.Customer.ADDRESS_FIELD_LABELS if (getattr(ed, f, None) or '').strip()]}, "
    ...
    if not ok and is_emergency and (ed.city or extracted_city):
        logger.info(
            f"🚑 EMERGENCY BYPASS: allowing incomplete address for {mask_chat_id(chat_id)} (city={ed.city or extracted_city})"
        )
        ok = True
    ...
    if not ok:
        ...
        if current_lead_id and partial_update:
            ...
            logger.info(
                f"💾 Persisted partial address parts for {mask_chat_id(chat_id)} (lead={current_lead_id}): {list(partial_update.keys())}"
            )

        await StateManager.set_state(chat_id, UserStates.AWAITING_ADDRESS)
        await whatsapp.send_message(chat_id, reason)
        logger.warning(
            f"🚫 Address gate REJECTED finalization for {mask_chat_id(chat_id)}: {reason}"
        )
        return
    logger.info(f"✅ Address gate PASSED for {mask_chat_id(chat_id)}")
    ...
    if lead:
        ...
        logger.info(
            f"Customer {mask_chat_id(chat_id)} entered AWAITING_PRO_APPROVAL state"
        )
    ...
    if await _is_registered_pro(chat_id):
        logger.info(
            f"Keeping pro-as-customer {mask_chat_id(chat_id)} on the customer side — "
