...
| `test_admin_responsive.py` | S1+S2 of the mobile pass: the three breakpoints exist and the `responsive.py` block is appended **last** inside `load_css`'s `<style>` (appended-last is what lets it win the cascade without more `!important`); no fixed `width: NNNpx !important` survives outside a media query (the sidebar rule that covered the whole phone screen); `render_metric_grid` escapes and preserves order; an empty Kanban column is marked and hidden only on a phone; the board wrapper is a class carrying an explicit `dir` (PRO-46) |
