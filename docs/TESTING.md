...
| `test_stop_workflow.py` | `stop_railway_services.yml`: the `confirm` step's shell is extracted verbatim from the parsed YAML and run under `bash` — the typed environment name is accepted modulo surrounding whitespace and letter case (the 2026-09-08 `'production '` phone-keyboard refusals), refused when empty, the other environment, a prefix, a typo, or inner-spaced, refused for an unknown target even when echoed back, and treated as data not shell; structural pins that both inputs reach the step through `env:` rather than `${{ }}`, that the `railway down` step keys its token/environment on `steps.confirm.outputs.target` rather than the raw input, and that `confirm` has no default |
