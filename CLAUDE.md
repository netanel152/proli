...
| `monitor_service.py` | Stale job detection, reassignment (shared `reassign_lead` helper), stale lead reminders (nudger), pro-approval SLA monitor (`check_pro_approval_sla` — nudges a silent pro, then offers the customer reassignment), escalation to PENDING_ADMIN_REVIEW, and Green API deauth detection (`check_whatsapp_instance_state`) |
