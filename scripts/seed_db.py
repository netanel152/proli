...
from app.core.constants import DEVELOPMENT_ENV, LeadStatus, STAGING_ENV
    ...
    for pro_id in pro_ids:
        for day_offset in range(7):
            ...
            for hour in range(8, 18):
                ...
                is_taken = random.choice(
                    [True, False, False]
                )  # 33% chance of being taken
...
async def create_leads(pro_ids):
    ...
    for _ in range(10):
        ...
        lead = {
            ...
            "issue_type": random.choice(
                ["נזילה", "קצר חשמלי", "התקנת גוף תאורה", "סתימה בכיור"]
            ),
    ...
    lead_booked = {
        ...
        - timedelta(hours=2),  # Booked 2 hours ago
    ...
    if messages_to_create:
        ...
        print(
            f"✅ Created {len(leads_to_create)} leads and {len(messages_to_create)} messages."
        )
...
async def create_pending_pro():
    ...
    print(
        f"   Created Pending Pro: {pending_pro['business_name']} ({result.inserted_id})"
    )
...
def assert_seed_allowed() -> None:
    """PRO-34: refuse to seed a production environment.

    ``seed()`` starts with a destructive ``clear_db()`` that empties every
    collection. Staging and development are legitimate targets; production
    never is. This runs before the first DB call so an operator whose shell
    points at production aborts instead of wiping it — the near-miss that
    motivated making ``staging`` a first-class environment.

    Written as an allow-list rather than ``if settings.is_production`` so it
    fails **closed**: any environment value added later is refused until
    someone deliberately adds it here.
    """
    if settings.ENVIRONMENT not in (DEVELOPMENT_ENV, STAGING_ENV):
        raise SystemExit(
            f"❌ Refusing to seed: ENVIRONMENT={settings.ENVIRONMENT!r}.\n"
            f"   Target database: {DB_NAME}\n"
            "   seed_db.py clears every collection before inserting fixtures, "
            "so it only ever runs against development or staging.\n"
            "   If this really is a staging deploy, fix ENVIRONMENT "
...
async def seed():
    assert_seed_allowed()
    print(
        f"Starting Seed Process... (environment={settings.ENVIRONMENT}, db={DB_NAME})"
    )
