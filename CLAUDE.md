...
|---|---|
| `workflow_service.py` | Central orchestrator — routes messages, manages FSM states, delegates to customer/pro/admin flows; handles emergency bypass and loyalty checks. The head of its dispatch runs `dispatch_guards.GUARD_CHAIN` first (PRO-179, first slice of the PRO-139 decomposition) |
| `dispatch_guards.py` | Ordered guard chain (`GUARD_CHAIN`, a tuple — order is load-bearing) for the first five branches of `workflow_service`'s dispatch: admin routing wizard, global reset, help/menu, inbound rate limit, zero-touch intent confirmation (PRO-179) |
